...
{
    stopThread();
    ROS_INFO_STREAM("DepthFilter destructed.");
}
...
{
    ROS_INFO_STREAM("DepthFilter stop thread invoked.");
    if(thread_ != NULL)
    {
	ROS_INFO_STREAM("DepthFilter interrupt and join thread... ");
...
void DepthFilter::initializeSeeds(FramePtr frame)
{
    ...
    feature_detector_->setExistingFeatures(frame->fts);
    feature_detector_->detect(frame.get(), frame->img_pyr,
				MIN_CORNER_SCORE, new_features);
    ...
    if(options_.verbose)
	ROS_INFO_STREAM("DepthFilter: Initialized "<<new_features.size()<<" new seeds");
    seeds_updating_halt_ = false;
}
...
void DepthFilter::reset()
{
    ...
    {
	lock_t lock(seeds_mut_);
	seeds_.clear();// Delete all seeds(feature pixel).
    }
    lock_t lock();
    while(!frame_queue_.empty())
	frame_queue_.pop();// Delete all frames stored before.
    seeds_updating_halt_ = false;
...
void DepthFilter::updateSeedsLoop() // Run in the thread.
{
	...
	FramePtr frame;
	{
		...
		frame_queue_cond_.wait(lock);// Wait until a frame added in.
	    if(new_keyframe_set_)// Once a keyframe is added, reset everything.
	    {
		new_keyframe_set_ = false;
		seeds_updating_halt_ = false;
		clearFrameQueue();
...
//   n_seeds = seeds_.size();
  ...
  const double focal_length = FOCAL_LENGTH;//charles
  double px_noise = 1.0;
  ...
  while( it!=seeds_.end())
  {
    ...
    SE3 T_ref_cur = it->ftr->frame->T_w_b.inverse() * frame->T_w_b;
    const Vector3d xyz_f(T_ref_cur.inverse()*(1.0/it->mu * it->ftr->f) );// transforms the point written in a homogeneous form to a normal coordinate.
    if(xyz_f.z() < 0.0)  {
      ++it; // behind the camera
      continue;
    }
    if(!frame->isInFrame(frame->f2c(xyz_f).cast<int>())) {//f2c transforms a point from world reference frame to camera reference frame.//charles
    ...
    float z_inv_max = max(it->mu - sqrt(it->sigma2), 0.00000001f);// Assert z_inv_max is bigger than zero.
    double z;
    //findEpipolarMatchDirect: After finding the best match of the patch, the depth z is computed by triangulation.
...
Vector3d xyz_world(it->ftr->frame->f2w(it->ftr->f * (1.0/it->mu)));// charles
      ...
      */
      {
	//seed_converged_cb_ = boost::bind(&MapPointCandidates::newCandidatePoint, &map_.point_candidates_, _1, _2)
...
void DepthFilter::updateSeed(const float x, const float tau2, Seed* seed)//Same as the derivation in the paper.
{
    ...
    float C1 = seed->a/(seed->a+seed->b) * boost::math::pdf(nd, x);//Returns PDF (density) at point x of distribution my_dist.
